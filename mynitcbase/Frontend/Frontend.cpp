...
int Frontend::select_attrlist_from_table(char relname_source[ATTR_SIZE],
    ...
    // Call appropriate project(But, before that we need to cover the FUNCTION command. This is a test command that is provided to you implement any other feature you want to implement. The Frontend User Interface will pass along any command beginning with FUNCTION to the Frontend::custom_function() method. The implementation of this method is left entirely to your imagination.) method of the Algebra Layer
...
int Frontend::select_attrlist_from_table_where(
    ...
    int ret=OpenRelTable::openRel(relname_source);  
    if(ret<0){
      return ret;
    }
    ...
    ret = Algebra::select(relname_source,temp,attribute,op,value);
